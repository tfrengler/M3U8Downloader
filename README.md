...
Just to clear: this is a learning project because I was curious how to download HLS media streams. It is by no means the optimal, smart or reliable way to do it.
In fact I discovered while writing this that ffmpeg can do the whole thing for you! That of course didn't stop me from trying anyway. So if you are interesting in doing something like this and a Google search brought you here, then hopefully you can use this as a resource to figure out your own way of doing it.

This will NOT work with every m3u8 manifest out there. I specifically worked on just the sample m3u8 file that's included in this repo which comes from https://insight-hls-amd1.akamaized.net/libraryitems-hls/d7w3j7p4u1v0u2b6e8w2y0k7z4m1n9g8t2s1g5e0/index.m3u8
There's lots of potential deviations that can happen and be taken into account. Here's a list of what it does, lessons learned and what to keep in mind:

# The m3u8 file included in this repo is session locked. That means it expires after a while. So rather than download it and re-use it for parsing/extracting it's better to target the url it and download it from source each time. Don't know if the session locking is standard practice for m3u8 playlists but it is on https://insighttimer.com/ apparently.
